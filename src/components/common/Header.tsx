...
function Header({ custom = false, back = true, icon, color, title, desc}) {
  ...
  return (
    <>
      <header className="sticky top-0 w-full px-5 py-2 bg-white shadow-sm flex justify-center">
        <button 
          type="button"
          onClick={() => navigate(-1)}
          className='absolute left-2 right-2 w-10 h-10 bg-brand-100 rounded-2xl grid place-content-center'>
